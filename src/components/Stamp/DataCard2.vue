<template>
    <div class="order-card-glow">
      <div class="order-card-main">
        <div class="line1-content flex w-full justify-start   ">
            <div class="catogory">{{ catogory }}</div>
            <div class="flex flex-col"><div class="box-label">工单号：</div><div class="text-sm text-white">{{ frame }}</div></div>
            <div class="flex flex-col"><div class="box-label">品号：</div><div class="text-sm text-white">{{ customerNo }} </div></div>
            <div class="flex flex-col"><div class="box-label">规格：</div><div class="text-sm text-white">{{ modelSpec }}</div></div>
            <div class="flex flex-col"><div class="box-label">计划数：</div><div class="text-sm text-white">{{ planNum }}</div></div>
          </div>
        <!-- <div class="data-grid">
        ...
        <!-- 进度条 -->
        <div class="progress-section flex w-full items-center mb-1">
    <div class="progress-label w-28 text-blue-200 text-right mr-2">割管完成进度</div>
    <div class="progress-bar flex-1 mx-2">
      <div class="progress-fill" :style="{ width: progressPercent + '%' }"></div>
    </div>
    <div class="progress-value w-10 text-white text-right">{{ progressPercent + '%' }}</div>
  </div>
  <div class="progress-section flex w-full items-center mb-1">
    <div class="progress-label w-28 text-blue-200 text-right mr-2">弯管完成进度</div>
    <div class="progress-bar flex-1 mx-2">
      <div class="progress-fill" :style="{ width: progressPercent + '%' }"></div>
    </div>
    <div class="progress-value w-10 text-white text-right">{{ progressPercent + '%' }}</div>
  ...
  <div class="progress-section flex w-full items-center mb-1">
    <div class="progress-label w-28 text-blue-200 text-right mr-2">冲孔完成进度</div>
    <div class="progress-bar flex-1 mx-2">
      <div class="progress-fill" :style="{ width: progressPercent + '%' }"></div>
    </div>
    <div class="progress-value w-10 text-white text-right">{{ progressPercent + '%' }}</div>
