<template>
    <div class="order-card-glow">
            ...
            <div class="main-row">
              <div class="params-block">
                <div class="param-row">
                  ...
                  <span class="param-std">标准：{{stdTemperature!==null && stdTemperature !== undefined && stdTemperature !== '' ? stdTemperature : '暂无标准' }}</span>
                  <span class="param-act">实际值：{{ temperature !== null && temperature !== undefined && temperature !== '' ? temperature : '暂无数据' }}</span>
                </div>
                <div class="param-row">
                  <span class="param-label">压力：</span>
                  <span class="param-std">标准：{{ stdPressure!==null && stdPressure !== undefined && stdPressure !== '' ? stdPressure : '暂无标准'  }}</span>
                  <span class="param-act">实际值：{{ pressure !== null && pressure !== undefined && pressure !== '' ? pressure : '暂无数据' }}</span>
                </div>
                <div class="param-row">
                  <span class="param-label">射速：</span>
                  <span class="param-std">标准：{{   stdMaxspeed!==null && stdMaxspeed !== undefined && stdMaxspeed !== '' ? stdMaxspeed : '暂无标准' }}</span>
                  <span class="param-act">实际值：{{ maxspeed !== null && maxspeed !== undefined && maxspeed !== '' ? maxspeed : '暂无数据' }}</span>
                </div>
                <div class="param-row">
                  <span class="param-label">保压时间：</span>
                    <span class="param-std">标准：{{stdKeeptime!==null && stdKeeptime !== undefined && stdKeeptime !== '' ? stdKeeptime : '暂无标准' }}</span>
                  <span class="param-act">实际值：{{ keeptime !== null && keeptime !== undefined && keeptime !== '' ? keeptime : '暂无数据' }}</span>
                </div>
              </div>
      ...
      <!-- 警告区域 -->
      <div v-if="missingStandardList.length" class="warning-area warning-info">
        <div class="warning-title">
          <span>ℹ 提示</span>
        </div>
        <div class="warning-content">
          以下参数标准暂无标准：{{ missingStandardList.join('、') }}
        </div>
      </div>
...
const hasActualData = (value) => {
  ...
  .order-card-glow {
    ...
    border-radius: 16px;
    box-shadow: 0 0 24px 4px #2575fc, 0 0 0 2px #2575fc inset;
  ...
  .order-card-block {
    ...
    border: 2px solid #2575fc;
    border-radius: 12px;
    box-shadow: 0 0 12px #00eaff99, 0 0 0 2px #2575fc inset;
    display: flex;
    flex-direction: column;
    align-items: center;
    justify-content: center;
  ...
  .warning-area {
    ...
    border-radius: 14px;
    box-shadow: 0 0 16px 2px #ffb30099, 0 0 0 2px #fffbe6 inset;
    color: #fff;
    z-index: 10;
  ...
  @media (max-width: 1200px) {
    ...
    .param-label, .param-std, .param-act {
      font-size: 0.9em;
    }
    .energy-block {
      padding: 8px 6px;
