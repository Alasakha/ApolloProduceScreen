<template>
  <div class="pipe-fitting-group h-full flex flex-col gap-3">
    <!-- 上部分：人员信息 -->
    <div class="personnel-section">
      <PersonnelPanel :title="'管件组'" :data="personnelData" />
    </div>
...
import { getGuanjianMachine, type GuanjianMachineItem } from '@/api/getStampWeldinfo'
...
// 将API数据转换为组件需要的格式
const transformEquipmentData = (apiData: GuanjianMachineItem[]) => {
...
const fetchData = async () => {
  try {
    loading.value = true
    const res = await getGuanjianMachine()
    ...
    if (res.code === 200) {
      ...
      console.error('获取关键设备数据失败:', res.message)
      equipmentList.value = []
    }
  } catch (error) {
    console.error('获取关键设备数据失败:', error)
    equipmentList.value = []
  } finally {
    loading.value = false
  }
...
const fetchPersonnelPanelData = async () => {
  try {
    const res = await getMetalworkingEfficiency(12) // 13代表管件组
    if (res.code === 200) {
      personnelData.value = {
