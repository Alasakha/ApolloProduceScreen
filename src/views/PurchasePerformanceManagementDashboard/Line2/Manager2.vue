...
const managerData = ref({
  monthly: {
    target: 16.0,      // 月度目标金额(万元)
    actual: 11.2,      // 月度实际金额(万元)
    percentage: 34.1   // 月度占比
  },
  yearly: {
    target: 192.0,     // 年度目标(万元)
    cumulative: 145.8, // 年度累计(万元)
    percentage: 34.3   // 年度占比
  },
  categories: [
    { name: '生产设备降本', amount: 6.8, progress: 60.7 },
    { name: '辅料降本', amount: 3.2, progress: 28.6 },
    { name: '服务降本', amount: 1.2, progress: 10.7 }
  ]
})
...
const showManagerDetail = () => {
  console.log('查看采购经理B月度详情')
  // 后续可以添加弹窗显示详细数据
}

// 查看年度详情
const showYearlyDetail = () => {
  console.log('查看采购经理B年度详情')
  // 后续可以添加弹窗显示详细数据
}

// 填写原因/对策
const showReasonDialog = () => {
  console.log('填写采购经理B月度原因/对策')
  // 后续可以添加原因对策填写弹窗
}

const showYearlyReasonDialog = () => {
  console.log('填写采购经理B年度原因/对策')
  // 后续可以添加原因对策填写弹窗
}
</script>
