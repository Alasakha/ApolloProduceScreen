...
const managerData = ref({
  monthly: {
    ...
    actual: 9.1,       // 月度实际金额(万元)
    percentage: 27.8   // 月度占比
  },
  yearly: {
    target: 192.0,     // 年度目标(万元)
    cumulative: 117.0, // 年度累计(万元)
    percentage: 27.4   // 年度占比
  },
  categories: [
    { name: '通用件降本', amount: 4.8, progress: 52.7 },
    { name: '外协降本', amount: 2.9, progress: 31.9 },
    { name: '其他降本', amount: 1.4, progress: 15.4 }
  ]
})
...
const showManagerDetail = () => {
  console.log('查看采购经理C月度详情')
  // 后续可以添加弹窗显示详细数据
}

// 查看年度详情
const showYearlyDetail = () => {
  console.log('查看采购经理C年度详情')
  // 后续可以添加弹窗显示详细数据
}

// 填写原因/对策
const showReasonDialog = () => {
  console.log('填写采购经理C月度原因/对策')
  // 后续可以添加原因对策填写弹窗
}

const showYearlyReasonDialog = () => {
  console.log('填写采购经理C年度原因/对策')
  // 后续可以添加原因对策填写弹窗
}
</script>
