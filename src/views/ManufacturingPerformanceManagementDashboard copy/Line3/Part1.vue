...
// 月度数据（常规客户总直通率）- 使用 newFPY 返回数据
  const totalPassRate = {
    target: manufacturingStore.regularTotalTarget ? (manufacturingStore.regularTotalTarget * 100).toFixed(1) : '--',
    actual: manufacturingStore.regularTotalActual ? (manufacturingStore.regularTotalActual * 100).toFixed(1) : '--',
    achievement: manufacturingStore.regularThroughput ? (manufacturingStore.regularThroughput * 100).toFixed(1) : '--'
  }
  
  // 年度数据（暂用同月度数据）
  const annualData = {
    target: totalPassRate.target,
    plan: totalPassRate.actual,
    achievement: totalPassRate.achievement
  }
