...
// import production store (old) removed — using manufacturingStore instead
...
// expose loading/error for template
const loading = manufacturingStore.loading
const error = ref(null)
// 计算属性：处理后的客户数据
const customerData = computed(() => {
  // 月度数据 - 使用 manufacturing store 的 newFPY 接口结果
  const monthly = {
    target: manufacturingStore.aClassTotalTarget ? (manufacturingStore.aClassTotalTarget * 100).toFixed(1) : '--',
    actual: manufacturingStore.aClassTotalActual ? (manufacturingStore.aClassTotalActual * 100).toFixed(1) : '--',
    achievement: manufacturingStore.aClassThroughput ? (manufacturingStore.aClassThroughput * 100).toFixed(1) : '--'
  }

  // 年度数据 - 暂用同月度（后端若提供年度字段可替换）
  const yearly = {
    target: monthly.target,
    cumulative: monthly.actual,
