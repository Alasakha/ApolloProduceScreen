<!-- 到货不及时TOP5 -->
<template> 
     <div ref="qualityIndicators_zzyk" class="chart-container"></div>
</template>
...
// 1. 响应式数据
const rawData = ref([]);
const qualityIndicators_zzyk = ref(null);
...
const drawIndicators = () => {
  if (!qualityIndicators_zzyk.value) return;

  // 确保 ECharts 初始化时父容器已有大小
  chartInstance = echarts.init(qualityIndicators_zzyk.value);
  updateChart();
