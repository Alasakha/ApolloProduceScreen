...
// 1. 响应式数据
const rawData = ref([]);
const qualityIndicators_zzek = ref(null);
...
const drawIndicators = () => {
  if (!qualityIndicators_zzek.value) return;

  // 确保 ECharts 初始化时父容器已有大小
  chartInstance = echarts.init(qualityIndicators_zzek.value);
  updateChart();
...
<template>
        <div ref="qualityIndicators_zzek" class="chart-container"></div>
</template>
