<template>
  ...
  import { getMistakesAnd0missionsPie,getMistakesAndOmissions } from '@/api/getQuiltyinfo';
  ...
  // 饼图数据加载
  const fetchData = () => {
    getMistakesAnd0missionsPie('CPJ').then(res => {
      const formatted = formatPieChartData(res.data, 'udf04', 'total');
      rawData.value = formatted.filter(item => item.value !== 0).sort((a, b) => b.value - a.value);
      isDataEmpty.value = rawData.value.length === 0;
      nextTick(() => {
        initChart();
        const option = createChartOption('本月错漏检累计次数', rawData.value);
        setOption(option);
  ...
  const handleChartClick = async (params: any) => {
    if (params && params.name) {
      ...
      try {
        const res = await getMistakesAndOmissions(selectedPurchaser.value);
