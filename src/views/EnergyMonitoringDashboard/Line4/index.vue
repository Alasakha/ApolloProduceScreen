<template>
  ...
  <div class="line3-container mt-2">
...
const energyData = computed(() => {
  console.log('🔍 Line4组件：接收到的原始数据:', props.data)
  console.log('🔍 Line4组件：数据条数:', props.data?.length || 0)
  
  // 检查数据中是否包含我们需要的两个气表代码
  if (props.data && props.data.length > 0) {
    const targetCodes = ['000025061801', '000025061802']
    const foundCodes = props.data.filter(item => targetCodes.includes(item.machCode))
    console.log('🔍 Line4组件：找到的目标气表数据:', foundCodes)
    console.log('🔍 Line4组件：所有数据的machCode:', props.data.map(item => item.machCode))
  }
  ...
  const filteredData = dataToProcess.filter(item => {
    // 只显示指定的两个气表
    const targetMachCodes = ['000025061801', '000025061802']
    const isTargetGas = targetMachCodes.includes(item.machCode)
    
    console.log(`🔍 数据过滤 - ${item.machName}(${item.machCode}):`, {
      isTargetGas,
      willInclude: isTargetGas,
      machCodeInTarget: targetMachCodes.includes(item.machCode)
    })
  ...
  const sortedData = processedData.sort((a, b) => {
    const indexA = targetMachCodes.indexOf(a.machCode)
    const indexB = targetMachCodes.indexOf(b.machCode)
